...
// -----------------------------------------------------------------------------------------------------
// @ OMANONLINE TailwindCSS Main Plugin
// -----------------------------------------------------------------------------------------------------
    ...
    }) =>
    {
        ...
        addComponents(
            _.fromPairs(_.map(options.themes, (theme, themeName) => [
                ...
                ))), (value, key) => [[`--omanonline-${e(key)}`, value], [`--omanonline-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
            ])),
        );
        ...
        const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) =>
        {
            ...
            const background = theme(`omanonline.customProps.background.${colorScheme}`);
            const foreground = theme(`omanonline.customProps.foreground.${colorScheme}`);
            ...
            return {
                [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                    ...
                    ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--omanonline-${e(key)}`, value], [`--omanonline-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                    ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--omanonline-${e(key)}`, value], [`--omanonline-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                },
            };
        });
            ...
            theme: {
                extend: {
                    ...
                    colors: _.fromPairs(_.flatten(_.map(_.keys(flattenColorPalette(normalizeTheme(options.themes.default))), (name) => [
                        [name, `rgba(var(--omanonline-${name}-rgb), <alpha-value>)`],
                        [`on-${name}`, `rgba(var(--omanonline-on-${name}-rgb), <alpha-value>)`],
                    ]))),
                },
                omanonline  : {
                    customProps: {
