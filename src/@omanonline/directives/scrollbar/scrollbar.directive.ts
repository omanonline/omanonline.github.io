...
import { ScrollbarGeometry, ScrollbarPosition } from '@omanonline/directives/scrollbar/scrollbar.types';
 ...
 */
@Directive({
    selector  : '[omanonlineScrollbar]',
    exportAs  : 'omanonlineScrollbar',
    standalone: true,
})
export class OmanOnlineScrollbarDirective implements OnChanges, OnInit, OnDestroy
{
    /* eslint-disable @typescript-eslint/naming-convention */
    static ngAcceptInputType_omanonlineScrollbar: BooleanInput;
    /* eslint-enable @typescript-eslint/naming-convention */

    @Input() omanonlineScrollbar: boolean = true;
    @Input() omanonlineScrollbarOptions: PerfectScrollbar.Options;
    ...
    {
        // Enabled
        if ( 'omanonlineScrollbar' in changes )
        {
            // Interpret empty string as 'true'
            this.omanonlineScrollbar = coerceBooleanProperty(changes.omanonlineScrollbar.currentValue);

            // If enabled, init the directive
            if ( this.omanonlineScrollbar )
            {
                this._init();
            }
        ...
        // Scrollbar options
        if ( 'omanonlineScrollbarOptions' in changes )
        {
            // Merge the options
            this._options = merge({}, this._options, changes.omanonlineScrollbarOptions.currentValue);
    ...
    isEnabled(): boolean
    {
        return this.omanonlineScrollbar;
    }
    ...
    private _init(): void
    {
        ...
        {
            this.omanonlineScrollbar = false;
            return;
        }
