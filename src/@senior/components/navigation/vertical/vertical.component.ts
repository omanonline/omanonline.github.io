...
import { SeniorNavigationItem, SeniorVerticalNavigationAppearance, SeniorVerticalNavigationMode, SeniorVerticalNavigationPosition } from '@senior/components/navigation/navigation.types';
import { SeniorVerticalNavigationAsideItemComponent } from '@senior/components/navigation/vertical/components/aside/aside.component';
...
import { SeniorScrollbarDirective } from '@senior/directives/scrollbar/scrollbar.directive';
...
@Component({
    selector       : 'senior-vertical-navigation',
    templateUrl    : './vertical.component.html',
    styleUrls      : ['./vertical.component.scss'],
    animations     : seniorAnimations,
    encapsulation  : ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    exportAs       : 'seniorVerticalNavigation',
    standalone     : true,
    imports        : [SeniorScrollbarDirective, NgFor, NgIf, SeniorVerticalNavigationAsideItemComponent, SeniorVerticalNavigationBasicItemComponent, SeniorVerticalNavigationCollapsableItemComponent, SeniorVerticalNavigationDividerItemComponent, SeniorVerticalNavigationGroupItemComponent, SeniorVerticalNavigationSpacerItemComponent],
})
export class SeniorVerticalNavigationComponent implements OnChanges, OnInit, AfterViewInit, OnDestroy
{
    ...
    @Input() appearance: SeniorVerticalNavigationAppearance = 'default';
    @Input() autoCollapse: boolean = true;
    @Input() inner: boolean = false;
    @Input() mode: SeniorVerticalNavigationMode = 'side';
    @Input() name: string = this._seniorUtilsService.randomId();
    @Input() navigation: SeniorNavigationItem[];
    @Input() opened: boolean = true;
    @Input() position: SeniorVerticalNavigationPosition = 'left';
    @Input() transparentOverlay: boolean = false;
    @Output() readonly appearanceChanged: EventEmitter<SeniorVerticalNavigationAppearance> = new EventEmitter<SeniorVerticalNavigationAppearance>();
    @Output() readonly modeChanged: EventEmitter<SeniorVerticalNavigationMode> = new EventEmitter<SeniorVerticalNavigationMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> = new EventEmitter<boolean>();
    @Output() readonly positionChanged: EventEmitter<SeniorVerticalNavigationPosition> = new EventEmitter<SeniorVerticalNavigationPosition>();
    @ViewChild('navigationContent') private _navigationContentEl: ElementRef;

    activeAsideItemId: string | null = null;
    onCollapsableItemCollapsed: ReplaySubject<SeniorNavigationItem> = new ReplaySubject<SeniorNavigationItem>(1);
    onCollapsableItemExpanded: ReplaySubject<SeniorNavigationItem> = new ReplaySubject<SeniorNavigationItem>(1);
    ...
    private _seniorScrollbarDirectives!: QueryList<SeniorScrollbarDirective>;
    private _seniorScrollbarDirectivesSubscription: Subscription;
    ...
    @HostBinding('class') get classList(): any
    {
        /* eslint-disable @typescript-eslint/naming-convention */
        return {
            'senior-vertical-navigation-animations-enabled'             : this._animationsEnabled,
            [`senior-vertical-navigation-appearance-${this.appearance}`]: true,
            'senior-vertical-navigation-hover'                          : this._hovered,
            'senior-vertical-navigation-inner'                          : this.inner,
            'senior-vertical-navigation-mode-over'                      : this.mode === 'over',
            'senior-vertical-navigation-mode-side'                      : this.mode === 'side',
            'senior-vertical-navigation-opened'                         : this.opened,
            'senior-vertical-navigation-position-left'                  : this.position === 'left',
            'senior-vertical-navigation-position-right'                 : this.position === 'right',
        };
        /* eslint-enable @typescript-eslint/naming-convention */
    }
    ...
    /**
     * Setter for seniorScrollbarDirectives
     */
    @ViewChildren(SeniorScrollbarDirective)
    set seniorScrollbarDirectives(seniorScrollbarDirectives: QueryList<SeniorScrollbarDirective>)
    {
        // Store the directives
        this._seniorScrollbarDirectives = seniorScrollbarDirectives;

        // Return if there are no directives
        if ( seniorScrollbarDirectives.length === 0 )
        {
            return;
        }

        // Unsubscribe the previous subscriptions
        if ( this._seniorScrollbarDirectivesSubscription )
        {
            this._seniorScrollbarDirectivesSubscription.unsubscribe();
        }

        // Update the scrollbars on collapsable items' collapse/expand
        this._seniorScrollbarDirectivesSubscription =
            merge(
                ...
                .subscribe(() =>
                {
                    ...
                    seniorScrollbarDirectives.forEach((seniorScrollbarDirective) =>
                    {
                        seniorScrollbarDirective.update();
                    });
                });
    }
    ...
    ngAfterViewInit(): void
    {
        ...
        setTimeout(() =>
        {
            ...
            if ( !this._navigationContentEl.nativeElement.classList.contains('ps') )
            {
                ...
                const activeItem = this._navigationContentEl.nativeElement.querySelector('.senior-vertical-navigation-item-active');
            ...
            else
            {
                ...
                this._seniorScrollbarDirectives.forEach((seniorScrollbarDirective) =>
                {
                    // Skip if not enabled
                    if ( !seniorScrollbarDirective.isEnabled() )
                    {
                        return;
                    }

                    // Scroll to the active element
                    seniorScrollbarDirective.scrollToElement('.senior-vertical-navigation-item-active', -120, true);
                });
            }
        });
    }
     ...
     * @param item
     */
    openAside(item: SeniorNavigationItem): void
    {
     ...
     * @param item
     */
    toggleAside(item: SeniorNavigationItem): void
    {
        // Toggle
    ...
    private _showOverlay(): void
    {
        ...
        this._overlay.classList.add('senior-vertical-navigation-overlay');

        // Add a class depending on the transparentOverlay option
        if ( this.transparentOverlay )
        {
            this._overlay.classList.add('senior-vertical-navigation-overlay-transparent');
        }
    ...
    private _showAsideOverlay(): void
    {
        ...
        this._asideOverlay.classList.add('senior-vertical-navigation-aside-overlay');
